...
class KmerVisualizer:
    ...
    def _build_figure(self, initial_colorscale: str = 'Magma', initial_log_scale: bool = False) -> go.Figure:
        ...
        blue_jet_black = [[0.0, 'blue'], [0.15, 'cyan'], [0.3, 'green'], [0.5, 'yellow'], [0.7, 'orange'], [0.9, 'red'], [1.0, 'black']]

        # --- Interactive Controls Definition ---
        available_colorscales = {
            'Magma': 'Magma', 'Viridis': 'Viridis', 'Plasma': 'Plasma',
            'Cividis': 'Cividis', 'Jet': 'Jet', 'Rainbow': 'Rainbow',
            'BlueJetBlack': blue_jet_black
        }

        colorscale_dropdown = dict(buttons=[dict(label=name, method="restyle", args=[{"colorscale": scale}]) for name, scale in available_colorscales.items()],
...
if __name__ == "__main__":
    ...
    # REMOVED the --use_360 argument as it's now an interactive option
