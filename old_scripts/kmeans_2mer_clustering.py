...
from cuml.decomposition import PCA as cuMLPCA # Using cuML's PCA
...
# --- CONFIGURATION ---
# The features are now the columns from the new input file.
FEATURES_TO_CLUSTER = ['phi_i', 'psi_i', 'phi_i+1', 'psi_i+1']
# The new format represents 2-mers (dihedral pairs)
K_VALUE = 2
# Periodic handling is still relevant for phi/psi angles
HANDLE_PERIODIC_DATA = True
MAX_K_TO_TEST = 10 # Max number of clusters to test for the elbow method
...
def load_and_prepare_data(filepath, feature_cols, handle_periodic):
    ...
    try:
        # Optimization: Load directly into a cuDF DataFrame to keep data on GPU
    ...
    # Create a unique identifier from the index
    identifiers = df.index.astype(str)
    
    # Select only the feature columns for processing
    data_points = df[feature_cols]

    if handle_periodic:
        print("Applying periodic transformation (cos/sin)...")
        # cuDF can perform these operations directly on the GPU
        transformed_data = cudf.DataFrame()
        # Note: Assuming input angles are in degrees
...
# --- OPTIMAL K-VALUE FINDER (Unchanged logic, minor text change) ---
def find_optimal_clusters(data, max_k):
    print("\nFinding the optimal number of clusters using the Elbow Method...")
    inertias = []
    # Ensure the upper bound for k is less than the number of samples
    ...
    plt.ylabel('Inertia')
    plt.title(f'Elbow Method For Optimal k (Input: 2-mer Angles)')
    
    plot_filename = f'elbow_plot_dihedrals.png'
...
# --- SAVE RESULTS (Adapted for new identifier) ---
def save_results(identifiers, labels, kmeans_model, config_params, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    print(f"\nSaving results to '{output_dir}/'")

    # Create results dataframe from identifiers (cuDF Series) and labels (CuPy array)
...
# --- VISUALIZATION (Adapted for cuML PCA) ---
...
if __name__ == "__main__":
    ...
    print(f"--- Running Dihedral Angle Clustering on GPU ---")
    ...
    # Ensure data is in CuPy array format for cuML
