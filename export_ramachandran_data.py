...
def process_central_residue(db_connection, central_residue: str) -> dict:
    ...
    returning a dictionary for a single 2D frequency grid.
    """
    ...
    return {
        ...
        "z": np.where(z_data > 0, z_data, None).tolist()
    }
...
def process_context(db_connection, central_residue: str, context: str) -> dict:
    ...
    returning a dictionary containing 20 individual 2D frequency grids.
    """
    if context not in ['before', 'after']:
        raise ValueError("Context must be 'before' or 'after'")

    # Correctly query for k+1-mers to find the context for k-mers
...
def main():
    ...
    try:
        ...
        print(f"--- Exporting data for all central residues of length: {args.central_k_value} ---")
        
        # Get the list of all unique k-mers of the desired central length
