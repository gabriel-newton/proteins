...
def process_context(db_connection, central_residue: str, context: str) -> dict:
    ...
    """
    if context == 'before':
        # Use LAG() to get the residue at the previous position on the same chain
        query = """
            WITH ContextData AS (
                SELECT
                    residue,
                    LAG(residue, 1) OVER (PARTITION BY chain_id ORDER BY position) as neighbor_residue,
                    tau_NA,
                    tau_AC
                FROM invariants
            )
            SELECT neighbor_residue, tau_NA, tau_AC
            FROM ContextData
            WHERE residue = ? AND neighbor_residue IS NOT NULL
        """
    elif context == 'after':
        # Use LEAD() to get the residue at the next position on the same chain
        query = """
            WITH ContextData AS (
                SELECT
                    residue,
                    LEAD(residue, 1) OVER (PARTITION BY chain_id ORDER BY position) as neighbor_residue,
                    tau_NA,
                    tau_AC
                FROM invariants
            )
    ...
    faceted_data = {}
    # Group by the neighboring residue, which the query now provides
    grouped = df.groupby('neighbor_residue')

    for neighbor_aa in AMINO_ACIDS:
        if neighbor_aa not in grouped.groups:
            continue

        residue_df = grouped.get_group(neighbor_aa)
...
def main():
    parser = argparse.ArgumentParser(description="Export CONTEXTUAL Ramachandran data for residues of a given length to JSON files.")
    ...
    try:
        ...
        print("Fetching central residues to process... (This may take a while)")
        ...
        for central_aa in tqdm(central_residues, desc="Processing Central Residues"):
            # 1. Process and save the "before" context data
            ...
            # 2. Process and save the "next" (after) context data
